...
For reference, the app stores data in two places. **IndexedDB** holds everything live: one database per **workspace** — `librept` for the trainer's own work and `librept_sandbox` for the sandbox they learn and experiment in ([`data/workspace.js`](src/data/workspace.js), TODO §40) — each with one object store per data schema (`schema2`, `schema3`, …) plus a `meta` store. The sandbox does not exist until somebody opens it, and its per-workspace `localStorage` keys carry a `__sandbox` suffix; what belongs to the PERSON rather than the data — theme, language, accepted terms, the trainer's own name — is shared by both and carries no suffix. **`localStorage`** holds only the small keys around it — `librept_active_session` (live session cache), `librept_active_timers`, `librept_workout_setup_draft`, `librept_read_notifications`, `librept-theme`, and `librept_terms_accepted` (first-run agreement). `librept_db` also lives there, but is **no longer a live store**: it is read once as the legacy import source when a device moves onto the IndexedDB engine, then left untouched as the rollback snapshot ([`data/storageNamespace.js`](src/data/storageNamespace.js)). `openpt_*` are older legacy keys migrated on first load.
...
- **Sample data lives in its own database, not in the trainer's.** There are two workspaces (TODO §40): the trainer's own work in `librept`, and a **sandbox** in `librept_sandbox` for learning and experimenting. A separate database rather than a flag on each record, because a connection pointing at one *cannot* write the other — the isolation is structural, not a check somebody has to remember. The switch is one menu item; it re-renders rather than reloading, tints the header for as long as you are in there, and takes nothing out with it: a backup written in the sandbox is refused on the way into your own work, and each workspace syncs to its own Drive file. Timers are the one thing that deliberately crosses: your own work beeps wherever you are, the sandbox beeps only in the sandbox.
- **One build carries every supported data schema at once — no release tags, no version switching.** There is no per-release hosting, no versioned URL path, and no upgrade/rollback UI (all dropped, TODO §16/§18): a build's identity for support purposes is its **commit SHA**, shown in the header stamp and copyable from the build-info dialog, alongside the **data schema** it is running (`data/migrationSteps.js`'s `CURRENT_SCHEMA_VERSION`) — two different axes that are never collapsed into one number. The only thing storage keys on is the data schema, and that axis lives in IndexedDB's own per-schema object stores (`schema2`, `schema3`, ...; see below), not in `localStorage` — `localStorage`'s single `librept_db` key survives only as the one-time legacy import source for a device upgrading onto this engine, never as a live bucket.

- **Schema changes are a chain of validated steps, never one big jump — for the one-time upcast; ongoing writes are a star, not a chain.** A stored legacy database is walked from its own `schemaVersion` to the current one through small pure `(state) => state` transforms ([`data/migrationSteps.js`](src/data/migrationSteps.js)), run by [`data/schemaMigrations.js`](src/data/schemaMigrations.js) on a **deep clone**, purely to upcast a legacy `localStorage` blob once at import time. Every step's output is **validated** before it counts; anything unrecognised **fails loud** and the original database is handed back untouched rather than written over. A database written by a **newer** build is refused outright instead of guessed at. Once upcast, ongoing writes go through the **star-write** layer instead ([`data/recordProjections.js`](src/data/recordProjections.js)): every record is projected directly from the live domain object into **every** live schema's IndexedDB store at once, in one transaction — no step feeds another, so error cannot compound and a "downgrade" is just a projection that was being written all along. See TODO §18.

- **The persistence engine is IndexedDB, not `localStorage`.** [`data/stateStore.js`](src/data/stateStore.js) keeps its read model synchronous (`getState()`/`setState()` return/replace a fully hydrated object, unchanged for every caller), but loading at boot and saving on write both go through [`data/indexedDb.js`](src/data/indexedDb.js) — one database, one object store per live schema major, so a single transaction can write every schema atomically. Boot does a one-time, revertable import from the legacy `localStorage` bucket (left untouched as the rollback snapshot), then reads back through the star-write projections. Falls back to plain `localStorage` if IndexedDB is unavailable. See TODO §18.6.
...
*   **Every view and record is addressable by a clean URL**, so links are shareable and bookmarkable and restore the same screen on load (the router's own design and invariants: [docs/ROUTING.md](docs/ROUTING.md)):
    ...
    *   `/session/{sessionId}/client/{clientId}/exercise/{exerciseId}` and `…/circuit/{circuitId}` — the clipboard with a specific card in focus. Opening the session upgrades the URL to whatever card is focused, and tapping a card updates it, so the address bar is always a copy-able link to the exact card on screen. When the trainer scrolls past the open card, it closes and the address ends in `/closed`, so a reload marks the same card without opening it.
    ...
    *   `/about`, `/terms`, `/build`, `/backup` — the globally-reachable **dialogs**. A dialog is a state a reload should restore, so it gets a URL: **Back closes it** (the dismiss gesture a phone user already knows), the ✕ and Escape agree because closing pops that entry, and a cold link reopens it over the dashboard. The mandatory first-run agreement is deliberately *not* routed — Back must not dismiss an agreement that has not been accepted.
