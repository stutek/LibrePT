...
- **A live Google canary that stores no secret** (TODO §1.5.1) — the hermetic suites pin our request shapes and structurally cannot see Google deprecating an endpoint or reclassifying a scope. A stored refresh token is exfiltratable by any workflow change *and* expires weekly while the OAuth app is in Testing mode, so **Workload Identity Federation** exchanges GitHub's OIDC assertion for a short-lived token at run time. Deliberately **not** a deploy gate: gating a release on a third party's uptime blocks a deploy for an outage no change of ours caused and no revert can fix. A test enforces the properties that would look harmless in a diff — no `pull_request_target`, no stored Google secret, no scope broader than production's two.
