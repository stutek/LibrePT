...
- **`agent_tools/wif_audit.py`** (TODO §1.5.1) — the two settings keyless CI rests on live in GCP, not in the repo, so no review can confirm them and both fail **silently**: everything works, the canary stays green, and strangers can mint tokens as our service account. Audits that the provider's `attributeCondition` pins `assertion.repository`, and that the service-account binding names one repository rather than the whole pool — the same hole one layer down, and the likelier mistake since it is what copy-pasted snippets show. Rejects conditions that look protective and are not: `!=` admits every repository but one; `startsWith` admits every repository an owner ever creates.
