...
- **Right to Erasure / "Right to be Forgotten" (Art. 17):** LibrePT honours an erasure request by **anonymising, not deleting**. Every identifying field on the client record is cleared, the client's name is removed from the training records that had copied it, and what remains — the sessions, the loads, the dates — is keyed to an opaque id that no longer resolves to a person. It is irreversible on purpose: a reversible scheme needs a mapping from pseudonym back to person, and with no server that mapping would sit in the same local database it is meant to protect. Two things this cannot reach, and the app says so rather than implying otherwise: a **backup taken before the erasure still names them** (LibrePT keeps a suppression entry so restoring one does not bring them back), and your own paper files and memory are outside any software's reach.
- **Retention (Art. 5(1)(e)):** You decide how long you keep client data, and you should tell your clients what you decided — that is your obligation as the controller, not the app's. What LibrePT does is this: **nothing is deleted on a schedule.** Records stay in your local database until you remove them or clear the app's data, and the anonymised remains of an erased client are kept **indefinitely**, because a training history with the person taken out of it is no longer personal data (Recital 26) and is what makes your own records of what you coached, and when, still complete. If your own retention policy is shorter than "indefinitely", act on it yourself: erase the client, and delete or re-take the backups that predate the erasure.
