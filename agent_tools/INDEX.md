...
| [constant_copies.py](constant_copies.py) | `diagnostic` | manual, in the duplication sweep | Each DECLARED shared constant's literal must appear only at its declaration — the dev-server port, the deployed site URL, the issue tracker. Inverts the obvious design on purpose: "no hardcoded ports anywhere" needs an ever-growing exemption list, while "this declared value appears once" needs none, and the list grows only when someone declares a shared constant. Catches RE-INTRODUCTION, not first introduction. Comments and docstrings count (TODO §28.1): a comment quoting a port is a copy that goes stale silently. Deliberately NOT a gate yet (TODO §28.3, AGENT_RULES §6.2) — it earns one by catching something twice. |
