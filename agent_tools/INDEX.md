...
| [wif_audit.py](wif_audit.py) | `diagnostic` | manual, after any WIF change | Audits the LIVE Workload Identity Federation setup, because the two settings the keyless-CI design rests on live in GCP rather than in this repo — nothing under version control can be reviewed to confirm them, and both fail **silently**: everything keeps working and the canary stays green while strangers can mint tokens as our service account. Checks that the provider's `attributeCondition` pins `assertion.repository` to this repo (absent = every GitHub Actions token on the internet is accepted) and that the service-account binding names `attribute.repository/<owner>/<repo>` rather than the whole pool, which re-opens the same hole one layer down and is what most copy-pasted snippets show. Rejects conditions that look protective but are not: `!=` admits every repository but one, `startsWith` admits every repository an owner ever creates. Needs network + authenticated `gcloud`, so it cannot gate `build check`; the analysis is unit-tested without either. |
