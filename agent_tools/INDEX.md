...
| [constant_copies.py](constant_copies.py) | `diagnostic` | manual, in the duplication sweep | Each DECLARED shared constant's literal must appear only at its declaration — the dev-server port, the deployed site URL, the issue tracker. Inverts the obvious design on purpose: "no hardcoded ports anywhere" needs an ever-growing exemption list, while "this declared value appears once" needs none, and the list grows only when someone declares a shared constant. Catches RE-INTRODUCTION, not first introduction. Comments and docstrings count (TODO §28.1): a comment quoting a port is a copy that goes stale silently. Deliberately NOT a gate yet (TODO §28.3, AGENT_RULES §8.2) — it earns one by catching something twice. |
...
| [test_assertions.py](test_assertions.py) | `check` | `build check` Stage 1 + `structure-checks` job | Verifies tests pin the BEHAVIOUR a caller depends on, not the mechanics that produce it (AGENT_RULES §4.8): exact multi-class strings, assertions on a stub's call count, identity equality standing in for "unchanged", and counters a stub keeps that nothing reads. Written because auditing 125 test files by hand costs more than the cleanup is worth and decays with the next test written — and it found a true positive the manual audit had missed on its first clean run. Each finding is cleared by NAMING the rule, which is what §4.8's three carve-outs require anyway; it is not an allowlist. Cannot see naming or over-mocking — those still need a reader. |
| [complexity.py](complexity.py) | `check` | `build check` Stage 1 | Cyclomatic (McCabe) complexity gate for `src/**/*.js`, via a real `tree_sitter_javascript` parse (not a regex heuristic — this codebase leans on `?.`/`??` throughout). No allowlist mechanism (AGENT_RULES §2.1) — every over-limit function fails the build unconditionally; split it instead. |
...
- [AGENT_RULES.md](../AGENT_RULES.md) — §8 states the build-a-tool rule these implement
