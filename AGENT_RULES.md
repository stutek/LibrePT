...
3. **Run the full local pipeline before every commit.** "Verified" means passing the same gate CI runs — not just that tests look green by eye. Before staging any commit, run `.venv/bin/python -m build check`, which runs `run_lint()` (Ruff + Biome lint **and** format checks) and `run_tests()` (the full suite) in one step. This is the entire point of the local/CI unification (see [TODO.md](TODO.md) §12.7): failures must surface **locally**, never for the first time on the push-triggered Pages deploy. Rules:
   ...
   - **Your change must add no new test failures.** Absolute green is the goal, but the repo may carry *pre-existing* failures you didn't cause; those must not block an unrelated commit. Establish the **baseline** (run the affected tests, or the whole suite, at `HEAD` before your change — a detached `git worktree` at the base commit is the clean way) and confirm your change introduces no failure that wasn't already there. Never silence a failure you can't explain.
   - **The full suite is flaky under parallelism.** `run_tests()` uses `pytest-xdist`; heavy fan-out produces spurious Playwright timeouts. Before believing a failure is real, **re-run the affected file serially** (`-p no:xdist`). A test that passes serially but fails in the parallel run is contention, not a regression.
   - Running only a subset (e.g. a single test file) is not sufficient verification for a commit — but it *is* the right tool for confirming or dismissing a specific failure.
