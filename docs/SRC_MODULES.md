...
| [src/data/recordSchemas.js](../src/data/recordSchemas.js) | `data` | Declared per-collection record shapes, per live schema major (TODO §18.1/§18.4) — the first place "schema N" exists as data rather than as a side effect of a migration transform. `fieldIssues`/`isRecordValid` are the structural validator every projection is held to. Also owns `READ_SCHEMA` (the schema this build reads, **declared not derived**) and `SCHEMA_PREVIEW`. |
| [src/data/previewSchema.js](../src/data/previewSchema.js) | `data` | Opt-in preview of an in-development data shape, in its own disposable IndexedDB database keyed by `BUILD_INFO.commit`: projected on opt-in, mirrored by the star-write fan-out, rebuilt when the commit changes or a projection was interrupted, deleted on opt-out. Restartable rather than resumable — the projection is one transaction, so an interruption leaves nothing to resume from. |
