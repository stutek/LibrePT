...
| [DEMO_CARD_COPY.md](DEMO_CARD_COPY.md) | `draft` | The demo story's narration cards, rewritten card by card with the maintainer: the agreed wording in both languages, the terminology one name per thing, and what each review found. Nothing here is shipped. |
...
| [SRC_MODULES.md](SRC_MODULES.md) | `index` | Catalog of every runtime module under `src/` — app entry, data layer, feature modules, controllers, service worker. Lives here, not in `src/`, because `src/` is copied wholesale into `dist/` on deploy, so documentation must stay out of the app tree. |
