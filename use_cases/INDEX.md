...
| **UC9** | [uc9_program_import.md](uc9_program_import.md) | Importing a Programme Written Elsewhere | A plan written in a chat window, a spreadsheet or a colleague's file, brought in without retyping: four optional inputs, every parsing failure reported BEFORE anything opens, and the result landing in the ordinary plan editor — which is what turns "parse correctly" into "parse usefully", since a wrong guess is a field somebody retypes rather than a record to repair. A movement the catalogue does not have is allowed and MARKED, never silently renamed into the nearest thing. |
