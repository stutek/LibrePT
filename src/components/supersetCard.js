...
export function renderSupersetCard(card, item, ctx) {
  ...
  if (showInFocus) {
    ...
    item.items.forEach((ex, idx) => {
      ...
      let repsHTML = '';
      if (isMax) {
        const currentLog = activeClientState && activeClientState.logs[ex.id] && activeClientState.logs[ex.id][round - 1];
    ...
    card.innerHTML = `
      ...
      ${footer}
    `;
    card.querySelectorAll('.superset-ex-row').forEach(rowEl => {
      const exId = rowEl.getAttribute('data-ex-id');
      rowEl.querySelector('.superset-sig.easy').addEventListener('click', (e) => { e.stopPropagation(); logQuickSignal('Too Easy - Increase Load', exId); });
      rowEl.querySelector('.superset-sig.hard').addEventListener('click', (e) => { e.stopPropagation(); logQuickSignal('Too Hard - Reduce Load', exId); });
      rowEl.querySelector('.superset-sig.note').addEventListener('click', (e) => { e.stopPropagation(); openFeedbackModal(exId); });
