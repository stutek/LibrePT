...
export function renderNotificationArea() {
  ...
  if (items.length > 0) {
    ...
    if (summaryIconEl && firstItem.icon) summaryIconEl.className = `${firstItem.icon} notification-bell-icon`;
  }
  ...
  container.querySelectorAll('button[data-nav-target]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (itemId && !readIds.includes(itemId)) {
        readIds.push(itemId);
        try { localStorage.setItem('librept_read_notifications', JSON.stringify(readIds)); } catch (e) {}
      }
  ...
  container.querySelectorAll('a[data-action-id]').forEach(link => {
...
export function setupNotificationGestures() {
  ...
  if (markAllBtn) {
    markAllBtn.addEventListener('click', (e) => {
      ...
      ['demo-welcome', 'spot-reservation-1', 'spot-cancellation-1'].forEach(id => {
