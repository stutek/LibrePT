...
export function renderClipboardEditor(container, deps) {
  ...
  items.forEach(it => {
    if (it.circuitId && !circuits.some(c => c.id === it.circuitId)) {
      circuits.push({ id: it.circuitId, title: it.circuitTitle || '', series: it.circuitSeries || 1 });
    }
  });
  const circuitMetaOf = (cid) => {
    const m = items.find(e => e.circuitId === cid && !isRest(e)) || items.find(e => e.circuitId === cid);
  ...
  for (let i = 0; i < items.length;) {
    ...
    if (it.circuitId) {
      ...
      const members = [];
      while (i < items.length && items[i].circuitId === cid) { members.push({ it: items[i], idx: i }); i++; }
  ...
  function normalizeCircuits() {
    ...
    const result = [];
    items.forEach(it => {
      if (!it.circuitId) { result.push(it); return; }
      if (emitted.has(it.circuitId)) return;               // members are pulled together on first sight
    ...
    Object.keys(rounds).forEach(cid => { if (!items.some(e => e.circuitId === cid)) delete rounds[cid]; });
    items.forEach(it => {
      if (!it.circuitId) return;
  ...
  listEl.querySelectorAll('.editor-rest-secs').forEach(input => {
    input.addEventListener('click', (e) => e.stopPropagation());
    input.addEventListener('change', () => {
  ...
  listEl.querySelectorAll('.editor-rest-remove').forEach(btn => {
  ...
  listEl.querySelectorAll('.editor-row-superset').forEach(sel => {
    ...
    sel.addEventListener('change', () => {
      ...
      if (val === '') {
        ...
        ex.circuitId = newId(); ex.circuitTitle = ''; ex.circuitSeries = DEFAULT_SERIES;
      } else {
  ...
  const makeExercise = (cid) => {
    ...
    return { id, name: '', setsTargetCount: 3, repsTarget: 10, weightTarget: 0,
      circuitId: cid || null, circuitTitle: meta ? meta.title : '', circuitSeries: meta ? meta.series : 1 };
  };
  const makeRest = (cid) => {
    const meta = cid ? circuitMetaOf(cid) : null;
    return { id: newId(), type: 'rest', rest: DEFAULT_REST,
      circuitId: cid || null, circuitTitle: meta ? meta.title : '', circuitSeries: meta ? meta.series : 1 };
  };
  listEl.querySelectorAll('.editor-insert').forEach(bar => {
    ...
    if (exBtn) exBtn.addEventListener('click', (e) => { e.stopPropagation(); items.splice(at, 0, makeExercise(cid)); commit(); });
    if (ssBtn) ssBtn.addEventListener('click', (e) => { e.stopPropagation(); items.splice(at, 0, makeExercise(newId())); commit(); });
    if (restBtn) restBtn.addEventListener('click', (e) => { e.stopPropagation(); items.splice(at, 0, makeRest(cid)); commit(); });
  });
  ...
  const rebuildFromDom = () => {
    const result = [];
    const pushRow = (el) => { const it = items[rowKeyOf(el)]; if (it) result.push(it); };
    listEl.querySelectorAll(':scope > li').forEach(li => {
      if (li.classList.contains('editor-circuit')) {
        li.querySelectorAll('.editor-circuit-list > li').forEach(r => { if (r.classList.contains('editor-row') || r.classList.contains('editor-rest-row')) pushRow(r); });
  ...
  listEl.querySelectorAll('.editor-reorder').forEach(handle => {
    const moveEl = handle.closest('.editor-row') || handle.closest('.editor-rest-row') || handle.closest('.editor-circuit');
