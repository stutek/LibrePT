...
- **Whole-app PREVIEW-STATE UI signal**: while running in `/preview/` (either tier), the app should be **unmistakably** marked as such — a slow-flashing warning treatment in the omnipresent app header (`components/applicationHeader.js`), potentially replacing the LibrePT logo itself for the duration, so there is no ambiguity about which build a PT (or a screenshot/bug report) is looking at. Open: exact visual treatment, and whether it also needs a corresponding non-visual signal (e.g. a build-stamp change, `renderBuildStamp()` in `app.js` already shows the commit SHA) for support/debugging purposes.
