...
3. **Edit-plan-from-review → schedule or leave unscheduled.** This assumes an "unscheduled session" exists at all — it currently doesn't. [uc5](use_cases/uc5_session_day_deck_and_deep_links.md) models every session as belonging to a day bucket (`yesterday/today/tomorrow/upcoming`); "unscheduled" is a new third state (no date), not a bucket value, and touches the day-deck, the router, and backup/restore round-trip, not just this one view.
...
8. **CLARIFIED — the dashboard day-deck itself becomes one continuous infinite deck.** Today's [daySelector.js](src/modules/sessionList/daySelector.js) is four discrete day-columns (`SESSION_DAY_ORDER = [yesterday, today, tomorrow, upcoming]`), one column filling the viewport, tab-like left/right paging, and deep links addressed by bucket name (not an arbitrary date — "upcoming" is a catch-all for everything past tomorrow). The new model replaces the four columns with **one continuous scroll axis ordered by time**, and turns the date picker from a tab-switcher into a **scrub-to-exact-time jump** — closer to a calendar scrubber than a tab bar. This is the single biggest change in this bundle: it touches the day-deck component, the swipe/fling settle logic (`SESSION_SCROLL_SETTLE_MS`, swipe-origin clamping), the deep-link route shape (`day` bucket → arbitrary date/timestamp), and virtualization (an infinite axis needs windowing, and "scrub to an exact future date" must work before that date's content has ever been rendered — needs anchor-point math, not just a scrollIntoView on an existing element).
...
**Sequencing implication**: (8) — the infinite-deck rewrite — is now the load-bearing prerequisite beneath everything else in this bundle: (3)'s unscheduled state needs somewhere to live in the new axis, (4)'s registry deck should reuse the same virtualized axis rather than inventing a second one, and (6)/(9)/half of (7) are all behaviors *of* that axis. Recommend landing (8) (continuous deck + scrub picker, no unscheduled/review behavior yet) as its own vertical slice first, then layering (3), (9), (4), (7) on top.
