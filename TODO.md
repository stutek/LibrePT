...
- **The field**: `position`, integer, **dense and unique — `0..n-1`** across one session's item list. Readers **sort by it**; embedded array order must agree but is not the authority, so a mismatch is a detectable defect instead of silent corruption.
- **Why now**: array order survives only while items are an embedded JSON array. Under §18's projections — or any store that holds items as rows — sequence is lost with nothing to rebuild it from, and the §18.3 completeness check would pass, because every id is present and only the *order* is wrong. That failure mode is indistinguishable from a scrambled program on the gym floor.
- **What it buys beyond order**: three assertions that the implicit scheme could not express — dense/unique (catches a dropped, duplicated or half-reordered item), array-agrees-with-position (catches a projection or import that re-sorted), and **circuit contiguity as a query** (`max(position) - min(position) + 1 === member count` per `circuitId`), which turns the invariant `normalizeCircuits()` upholds by convention into something verifiable at rest.
- **Dense, not gapped or fractional**: renumbering the tail on insert is the *same single record write* while items stay embedded, and sparse/lexicographic ranks would forfeit the intactness check — any set of distinct values is a valid rank set, so "is this list whole?" stops being answerable. Fractional ranks pay off for concurrent writers on row storage; a local-first single-trainer app has neither.
...
- **Sequencing (expand-first, per §18)**: (1) write `position` on every item in every writer — editor commits, deck inserts, `buildProgramSnapshot`, `openSessionFromHistory`, routine seeds, backup import — while readers still use array order; (2) add the three invariant checks + a repair that renumbers from array order; (3) flip readers to sort by `position`, keeping array order as the fallback for records written before it; (4) only then may storage stop guaranteeing array order.
- **Touches**: [sessionItemRecord.js](src/modules/common/sessionItemRecord.js), [clipboardEditor.js](src/modules/clipboard/clipboardEditor.js) (`normalizeCircuits`, `rebuildFromDom`, every splice), [exerciseDeck.js](src/modules/clipboard/exerciseDeck.js) (`insertFastAdjustment`), [activeSessionController.js](src/controllers/activeSessionController.js) (`injectExerciseIntoActivePlan`, `buildSupersetUnits`, and `activeExerciseIndex` — a positional pointer that must keep meaning an *array index*, not a position value), backup round-trip, `DEFAULT_ROUTINES`/`DEFAULT_HISTORY` seeds.
- **Back-compat**: additive behind a shape guard, exactly like `type`/`completed` in 17.1 — an item with no `position` falls back to its array index, so legacy history and old backups stay readable.
