...
2. **"PT decides when a review is addressed"** — read literally, this is a *manual* dismiss, independent of whether every underlying feedback tag was individually resolved via the existing adjustment wizard. That's two different completion signals that could drift out of sync (session marked reviewed while a tag underneath is still unresolved, or vice versa). Needs one clear rule before implementation, not both left implicit.
...
6. **Unscheduled-session ordering: after past/active, before future.** Fine as a stable tiebreak rule, but only meaningful once (3) exists.
...
**Sequencing implication**: (3) unscheduled sessions is the load-bearing prerequisite — (4), (6), and half of (7) build on it existing. Suggest building in that order rather than in parallel.
