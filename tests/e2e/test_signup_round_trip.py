...
def test_a_stranger_becomes_a_client_without_the_trainer_typing_anything(
    ...
    # Durable before reloading — the save is enqueued and the next line is a full page load, so this
    # is the difference between asserting "it survived" and racing the write (tests/conftest.py).
    wait_for_stored_record(page, "clients", {"name": "Nova Oseba"})
