...
const EDGE_CASES = {
  ...
  huge_sessions_array: `{ "schemaVersion": 2, "sessions": ${JSON.stringify(
...
test("migration never throws and always returns a well formed result", () => {
  ...
  for (const [name, outcome] of Object.entries(results)) {
    ...
    assert.equal(
      outcome.stateIsOriginalOnRefusal,
      true,
      `${name}: refusal fabricated a different state`,
    );
    assert.equal(
      outcome.stateIsWellFormedOnSuccess,
      true,
      `${name}: success produced a malformed state`,
    );
