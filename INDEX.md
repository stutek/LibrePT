...
| [src/data/recordProjections.js](src/data/recordProjections.js) | `data` | The star-write projection layer for the one schema currently live (TODO §18.1): one function per collection, domain object → IndexedDB record. `projectionIssues()` is what proves a projection is total. |
